...
API_KEY = "tzpGBefZV-T4kM3RzUALveNIAZVR_JwRhIs5q_gF3oEA"
...
def prediction():
  if request.method == 'POST':
    ...
    total = [[name, month, day_of_month, day_of_week,int(dept15), arrtime, origin1,origin2,origin3,origin4,origin5,destination1,destination2,destination3,destination4,destination5]]
    print(total)
    payload_scoring = {"input_data": [{"field": ['f0', 'f1', 'f2', 'f3', 'f4', 'f5', 'f6', 'f7', 'f8','f9', 'f10', 'f11', 'f12', 'f13', 'f14', 'f15'], "values": total}]}

    response_scoring = requests.post('https://eu-gb.ml.cloud.ibm.com/ml/v4/deployments/052352e2-334c-4ec8-a805-766c444c844f/predictions?version=2022-11-17', json=payload_scoring,
...
def sendpassword():
  if request.method == 'POST':
    ...
    if not account:
      ...
      msg = Message(subject='OTP', sender='xxxxxxxx',recipients=[email])
...
def register():
  ...
  if request.method == 'POST':
    ...
    if account:
      ...
      email = Email
      msg = Message(subject='OTP', sender='xxxxxx',
                    recipients=[email])
